...
export const requestTimeout = (_req, res, next) => {
...
export function sanitize(obj) {
    if (obj && typeof obj === "object") {
        ...
        for (const [k, v] of Object.entries(obj)) {
            clone[k] = /pass|secret|token|key/i.test(k) ? "***" : sanitize(v);
        }
        return clone;
    }
    return obj;
...
const boundedArray = (t) => z.array(t).max(500);
const boundedObject = (lazySelf) => z.record(z.string().min(1).max(128), z.union([primitive, boundedArray(primitive), z.lazy(lazySelf), boundedArray(z.lazy(lazySelf))])).refine(obj => Object.keys(obj).length <= 100, { message: "Too many keys" });
export const attributeValueSchema = z.lazy(() => z.union([primitive, boundedArray(primitive), boundedObject(() => attributeValueSchema), boundedArray(boundedObject(() => attributeValueSchema))]));
