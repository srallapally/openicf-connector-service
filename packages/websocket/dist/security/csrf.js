 ...
 */
export function loadCsrfConfig() {
    ...
    if (!signingSecret) {
        ...
        if (process.env.NODE_ENV === "production") {
            console.warn("[CSRF] WARNING: CSRF_SIGNING_SECRET not set in production. " +
    ...
    if (!enabled) {
        console.warn("[CSRF] WARNING: CSRF protection is DISABLED. " +
 ...
 */
export function verifyCsrfToken(signedToken, secret) {
 ...
 */
export function validateOrigin(originHeader, allowedOrigins, requestHost) {
    if (!originHeader) {
        return false;
    }
...
export function csrfProtection(config) {
    return (req, res, next) => {
        ...
        if (!validateOrigin(origin, config.allowedOrigins, host)) {
            console.warn(`[CSRF] Origin validation failed: origin=${origin}, host=${host}, ` +
        ...
        if (!tokenFromHeader) {
            console.warn(`[CSRF] Token missing from header: method=${method}, path=${req.path}`);
            return res.status(403).json({
        ...
        if (!tokenFromCookie) {
            console.warn(`[CSRF] Token missing from cookie: method=${method}, path=${req.path}`);
            return res.status(403).json({
        ...
        if (!verifyCsrfToken(tokenFromHeader, config.signingSecret)) {
            console.warn(`[CSRF] Invalid token signature: method=${method}, path=${req.path}`);
            return res.status(403).json({
        ...
        if (tokenFromHeader !== tokenFromCookie) {
            console.warn(`[CSRF] Token mismatch: method=${method}, path=${req.path}`);
            return res.status(403).json({
 ...
 * @returns Express middleware function
 */
export function csrfTokenEndpoint(config) {
    return (req, res) => {
 ...
 */
export function validateWebSocketOrigin(originHeader, config, requestHost) {
    // CSRF Protection Enhancement: Skip if disabled (development only)
    if (!config.enabled) {
        return true;
    }
    ...
    if (!isValid) {
        console.warn(`[CSRF] WebSocket connection rejected: Invalid origin=${originHeader}, ` +
            `host=${requestHost}`);
    }
    return isValid;
}
