...
export class ConnectorFacade {
  ...
  private call<T>(p: Promise<T>): Promise<T> { return this.breaker.exec(() => p); }

  async test(): Promise<void> { if (this.impl.test) await this.call(this.impl.test()); }
  ...
  async search(objectClass: string, filter: any, options?: OperationOptions) {
    if (typeof this.impl.search === "function" && this.impl.search.length >= 3) {
      ...
      const sr: SearchResult = await this.call((this.impl as any).search(objectClass, filter, handler, options));
