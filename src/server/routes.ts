...
import type { OperationOptions, ConnectorSpi } from "../spi/types.js";
...
// ---- Helpers ----
function cleanOptions(o?: unknown): OperationOptions | undefined {
  if (!o || typeof o !== "object") return undefined;
  return Object.fromEntries(Object.entries(o as Record<string, unknown>).filter(([,v]) => v !== undefined)) as OperationOptions;
}

function getSpi(registry: ConnectorRegistry, id: string | undefined): Partial<ConnectorSpi> | null {
  if (!id) return null;
  const inst: any = registry.get(id);
  if (!inst) return null;
  ...
  for (const c of candidates) {
    if (typeof c?.schema === "function" || typeof c?.search === "function" ||
      ...
      return c as Partial<ConnectorSpi>;
    }
  }
  return null;
}
...
export function buildRouter(registry: ConnectorRegistry) {
  ...
  r.param("objectClass", (req: Request, res: Response, next: NextFunction, value: string) => {
    if (value && value.startsWith("_")) return res.status(404).json({ error: "Unknown path" });
    return next();
  });
  ...
  r.post("/:id/_sync", async (req, res, next) => {
    try {
      ...
      if (!spi) return res.status(404).json({ error: "Connector not found" });
      if (!spi.sync) return res.status(501).json({ error: "Sync not implemented" });
  ...
  r.post("/:id/:objectClass/_search", async (req, res, next) => {
    try {
      ...
      const { id, objectClass } = req.params as { id: string; objectClass: string };
      const spi = getSpi(registry, id);
      if (!spi) return res.status(404).json({ error: "Connector not found" });
      if (!spi.search) return res.status(501).json({ error: "Search not implemented" });
  ...
  r.post("/:id/:objectClass/:uid/_addAttributeValues", async (req, res, next) => {
    try {
      ...
      const { id, objectClass, uid } = req.params as { id: string; objectClass: string; uid: string };
      const spi = getSpi(registry, id);
      if (!spi) return res.status(404).json({ error: "Connector not found" });
      if (!spi.addAttributeValues) return res.status(501).json({ error: "UpdateAttributeValuesOp not implemented" });
  ...
  r.post("/:id/:objectClass/:uid/_removeAttributeValues", async (req, res, next) => {
    try {
      ...
      const spi = getSpi(registry, id);
      if (!spi) return res.status(404).json({ error: "Connector not found" });
      if (!spi.removeAttributeValues) return res.status(501).json({ error: "UpdateAttributeValuesOp not implemented" });
  ...
  r.post(`/:id/${oc}`, async (req, res, next) => {
    try {
      ...
      const spi = getSpi(registry, id);
      if (!spi) return res.status(404).json({ error: "Connector not found" });
      if (!spi.create) return res.status(501).json({ error: "Create not implemented" });
  ...
  r.get(`/:id/${oc}/:uid`, async (req, res, next) => {
    try {
      ...
      const spi = getSpi(registry, id);
      if (!spi) return res.status(404).json({ error: "Connector not found" });
      if (!spi.get) return res.status(501).json({ error: "Get not implemented" });
      const out = await spi.get(objectClass, uid, req.query as any);
  ...
  r.patch(`/:id/${oc}/:uid`, async (req, res, next) => {
    try {
      ...
      const spi = getSpi(registry, id);
      if (!spi) return res.status(404).json({ error: "Connector not found" });
      if (!spi.update) return res.status(501).json({ error: "Update not implemented" });
  ...
  r.delete(`/:id/${oc}/:uid`, async (req, res, next) => {
    try {
      ...
      const spi = getSpi(registry, id);
      if (!spi) return res.status(404).json({ error: "Connector not found" });
      if (!spi.delete) return res.status(501).json({ error: "Delete not implemented" });
      await spi.delete(objectClass, uid, req.query as any);
      res.status(204).end();
    } catch (e) { next(e); }
  });
  ...
  r.get("/:id/schema", async (req, res, next) => {
    try {
      ...
      const spi = getSpi(registry, id);
      if (!spi) return res.status(404).json({ error: "Connector not found" });
      if (!spi.schema) return res.status(501).json({ error: "Schema not implemented" });
  ...
  r.post("/:id/_test", async (req, res, next) => {
    try {
      ...
      const spi = getSpi(registry, id);
      if (!spi) return res.status(404).json({ error: "Connector not found" });
      if (!spi.test) return res.status(501).json({ error: "Test not implemented" });
      await spi.test();
      res.status(204).end();
    } catch (e) { next(e); }
  });
